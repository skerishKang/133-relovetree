...
- [ ] 로그인 후, Neon/Postgres `trees` 테이블(compat API 경유) 기준으로 본인 소유 트리 목록이 로드되는지 확인
...
- [ ] Neon/Postgres trees 테이블에 일정 개수의 트리가 있을 때, "지금 뜨는 러브트리" 카드가 노출되는지 확인
- [ ] 카드 하단에 다음 정보가 보이는지 확인
  - `N개의 순간 · 조회 V · 좋아요 L · 공유 S`
- [ ] 좋아요/조회/공유 수가 높은 트리가 상단에 더 자주 보이는지 (랭킹 스코어 반영) 대략적인 감으로 확인
- [ ] DB 쿼리 실패 시 기본 데모 카드(정적 아티스트 카드 fallback)로 폴백되는지 확인

### 1-5. 최근 만들어진 러브트리 섹션

- [ ] Neon/Postgres `trees` 테이블에 트리가 없을 때: 섹션 전체가 렌더링되지 않고 숨겨지는지 확인
...
1. **뷰 카운트(viewCount)**
   - [ ] 특정 트리의 Neon/Postgres 레코드에서 현재 `viewCount` 값 확인
   - [ ] 에디터에서 해당 트리를 새 탭 또는 새 창에서 열기
   - [ ] DB 레코드를 새로고침하여 `viewCount` 가 1 증가했는지 확인
   - [ ] 여러 번 새로고침 시 viewCount가 누적 증가하는지 확인

2. **공유 카운트(shareCount)**
   - [ ] 에디터 상단 `Share` 버튼 클릭 → 링크 복사 토스트가 뜨는지 확인
   - [ ] Neon/Postgres에서 해당 트리의 `shareCount` 값이 1 증가했는지 확인

3. **좋아요(likes/likeCount)**
   - [ ] 로그인한 상태에서 하트 버튼 클릭 시 색상/배경이 바뀌고 `like-count` 숫자가 증가하는지 확인
   - [ ] DB 레코드에 `likes` 배열에 본인 uid가 들어가고, `likeCount` 값이 일관되게 반영되는지 확인
   - [ ] 한 번 더 누르면 좋아요 해제가 되고, DB에도 반영되는지 확인
...
- [ ] 에디터에서 트리를 엽니다.
- [ ] Neon/Postgres 레코드에 `lastOpened` 필드가 타임스탬프로 기록되는지 확인
...
- [ ] 데모 커뮤니티 글 생성 기능이 있다면, 글/댓글/좋아요 수가 UI/DB 모두에서 맞는지 확인
...
- [ ] Role 드롭다운을 변경했을 때 `role`, `isDemo`, `isAiBot` 플래그가 DB에 기대대로 저장되는지 확인
...
- [ ] Neon/Postgres에서 임의의 여러 트리를 확인했을 때, 각 레코드의 `nodeCount`가 `nodes.length`와 일치하는지 샘플 확인
...
- [ ] Neon/Postgres 보안 규칙 상, 비로그인/비관리자 상태에서 민감 데이터에 접근이 막혀 있는지 다시 한 번 샘플 확인
