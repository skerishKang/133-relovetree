...
| 항목 | 유형 | 근거 |
|------|------|------|
| `src/` | 폴더 | 메인 소스 코드 (auth.js, shared.js, firebase-firestore-compat.js, payment.js, input.css) |
...
| `*.html` | 파일 | 메인 웹 애플리케이션 (index.html, editor.html, community.html, owner.html, login.html, admin.html) |
| `*.js` | 파일 | 클라이언트 로직 (index.js, community.js, owner.js, editor_ai.js, admin.js, sw.js) |
| `output.css` | 파일 | 빌드된 Tailwind CSS |
| `firebase.json`, `.firebaserc` | 파일 | Firebase 설정 |
| `firestore.rules`, `storage.rules` | 파일 | Firebase 보안 규칙 |
| `netlify.toml` | 파일 | Netlify 빌드 설정 |
| `tailwind.config.js`, `postcss.config.js` | 파일 | CSS 빌드 설정 |
| `package.json`, `package-lock.json` | 파일 | npm 의존성 |
| `README.md`, `TUTORIAL.md`, `TEST_PLAN.md`, `POSTGRES_MIGRATION.md` | 파일 | 프로젝트 문서 |
