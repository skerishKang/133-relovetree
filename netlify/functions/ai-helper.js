/**
 * AI Helper - YouTube + AI Node Builder
 * 
 * This file is now a thin wrapper that delegates to modular sub-components:
 * - ai/service.js: Core AI text and node building
 * - ai/youtube.js: YouTube API wrappers  
 * - ai/helpers.js: Utility functions
 * 
 * Original handler logic preserved for backwards compatibility.
 */
const { getCorsHeaders } = require('./_lib/http');
const aiService = require('./_lib/ai/service');
const aiYoutube = require('./_lib/ai/youtube');
const { clampNumber, safeJsonParse, isoToDate, truncateText } = require('./_lib/ai/helpers');

const youtubeSearchFirstVideo = aiYoutube.youtubeSearchFirstVideo;
const youtubeSearchVideos = aiYoutube.youtubeSearchVideos;
const youtubeGetVideoDetails = aiYoutube.youtubeGetVideoDetails;
const getYouTubeApiKey = aiYoutube.getYouTubeApiKey;
const getTranscriptText = aiYoutube.getTranscriptText;
const { callAiText, buildRealNodeFromVideo, buildPromptBody, getAiKeys } = aiService;
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (mode === 'youtube_search') {
      ...
      const queryRaw = (payload && (payload.query || payload.q || payload.prompt)) || '';
    ...
    const keys = getAiKeys(process.env);
    if (!keys.length) {
      return {
        statusCode: 500,
    ...
    if (mode === 'qa') {
      ...
      for (const item of items) {
        ...
        const candidates = [
          rawSearchQuery, baseTitle, rawPrompt,
          `${baseTitle} 무대`, `${baseTitle} 직캠`, `${baseTitle} Mcountdown`,
          `${rawPrompt} 무대`
        ].map(v => (v || '').trim()).filter((v, idx, arr) => v.length > 0 && arr.indexOf(v) === idx);

        let found = null;
        for (const q of candidates) {
          try { found = await youtubeSearchFirstVideo(q, ytKey, process.env); } catch (e) { found = null; }
          if (found && found.videoId) break;
        }

        if (!found || !found.videoId) {
          built.push({ title: baseTitle || '새 순간', date: '', videoId: '', description: '', moments: [] });
          continue;
        }

        let details = null;
        try { details = await youtubeGetVideoDetails(found.videoId, ytKey, process.env); } catch (e) { details = found; }

        let transcriptText = '';
        try { transcriptText = await getTranscriptText(found.videoId); } catch (e) { transcriptText = ''; }

        const nodeObj = await buildRealNodeFromVideo({
          baseTitle, instruction: '', video: details || found, transcriptText, env: process.env,
        });
      ...
      const searchFallbackQueries = Array.isArray(payload && payload.searchFallbackQueries)
        ? payload.searchFallbackQueries.map(q => q == null ? '' : String(q).trim()).filter(q => q.length > 0).slice(0, 6)
        : [];
      ...
      if (videoId) {
        ...
        searchFallbackQueries.forEach(q => { if (q && !candidates.includes(q)) candidates.push(q); });
        if (trimmedTitle) candidates.push(trimmedTitle);
        if (trimmedSearch) candidates.push(`${trimmedSearch} 무대`, `${trimmedSearch} 직캠`);
        if (trimmedTitle) candidates.push(`${trimmedTitle} 무대`);
        const safeCandidates = candidates.map(q => q == null ? '' : String(q).trim()).filter(q => q.length > 0).slice(0, 8);
      ...
      if (found && found.videoId) {
        ...
        try {
          nodeObj = await buildRealNodeFromVideo({
            baseTitle, instruction,
            video: Object.assign({}, details || {}, { videoId: found.videoId }),
            transcriptText, env: process.env,
          });
        } catch (e) { nodeObj = null; }
        ...
        if (nodeObj && typeof nodeObj === 'object') {
          if (!nodeObj.videoId && !nodeObj.youtubeUrl) nodeObj.videoId = found.videoId;
          if (!nodeObj.title) nodeObj.title = baseTitle || (details && details.title) || '새 순간';
          if (!nodeObj.description) nodeObj.description = details ? truncateText(details.description, 240) : '';
        }
        result = nodeObj || {};
      } else {
        const rawText = await callAiText(promptBody.text, process.env);
